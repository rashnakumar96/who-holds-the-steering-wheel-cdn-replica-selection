...
def plotrttCDFsMixedApproach(country,results,resolver_dict,vantagePoints,cdn):
	...
	for resolverVantage in results[cdn]:
		...
		non_anycastcustomer["local"]=rtts
	...
	def plotDiffCustomers(customer,resolver_dict,cdn,country,customerType):
		...
		ind=0
		for resolverVantage in customer:
	...
	for resolverVantage in anycastcustomer:
		...
		# total.append(len(anycastcustomer[resolverVantage])+len(non_anycastcustomer[resolverVantage]))
	...
	print ("non-anycastcustomer",100*non_anycastTotal/(anycastTotal+non_anycastTotal))
	print ("\n")
...
def plotrttCDFs(country,results,resolver_dict,vantagePoints):
	...
	# countries_remove=["GB","FR","ES","TR","RU","DE","CN","IN","ID","AE","DZ","ZA","EG","GH","NG"]
...
def groupedBoxPlot(plot_x,plot_sameRegion,plot_diffRegion,fname):
 ...
 #            hue = data['sex'])
	...
	df = pd.DataFrame({'CDNs':plot_x,\
                  'Same Region':plot_sameRegion,'Different Region':plot_diffRegion})
	df = df[['CDNs','Same Region','Different Region']]


	plt.figure(figsize=(15,8))
	dd=pd.melt(df,id_vars=['CDNs'],value_vars=['Same Region','Different Region'],var_name='Region Type')
...
def KSThreshold(countries,distance_dict):
	...
	countriesPerRegion={"North America":["US"],"South America":["BR","AR"],"Asia":["IN","CN","ID","AE"],"Europe":["FR","GB","TR","RU","DE","ES"],"Oceania":["AU"],"Africa":["ZA","EG","NG","GH","DZ"]}
	...
	for country in countries:
		...
		for cdn in cdnResourcesMap:
			try:
				...
				print (country,cdn,"resourcesSizeType file not found")
				continue
			for resource in resourceSizeType[cdn]:
				try:
...
def internetUsersMap():
	...
	"GB":65045228,"FR":60421689,"TR":72500000,"RU":124630000,"DE":79127551,"ES":43509182,
	"CN":1010740000,"IN":833710000,"ID":212354070,"AE":12176400,
	"ZA":34545165,"EG":54741493,"NG":154301195,"GH":14767818,"DZ":37836425,
	"AU":23391152
	}
...
if __name__ == "__main__":
	...
	# cdns=["Cloudflare","Google","Akamai","Cloudfront","Fastly","Highwinds","BunnyCDN","CDN77","StackPath","EdgeCast"]
...
# #imp two commands to collect results and plot the graphs.
	...
	# countries=["US","BR","AR","GB","FR","ES","TR","RU","DE","CN","IN","ID","AE","AU"]
	# countries=["BR","US"]
